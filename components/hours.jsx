...
function Hours({db, auth , outterReset}) {
    ...
    function initUpdHours(){
        const updHours = hours.map(hour=>{
            ...
            return {
                ...hour,
                msTime,
                className: dateNow + 1*60*60*1000 - msTime <= 0? 'free': 'past'
            }
        })
    ...
    function resetUpdHours(){
        ...
        for (let idx = 0; idx < updHours.length; idx++) {
            if(["reserved", "past"].includes(newUpdHours[idx].className)){
    ...
    const onClick = (e)=>{
        ...
        if(['reserved', 'past', "border", "noaccess"].includes(className)){
            console.log('denied')
            return
        }
