...
export default async(req, res) => {
    ...
    try {
        ...
        const reservations = await ReservationSchema.find({userId: user._id, isPaid: true}).exec()

        res.status(200).json({
            message: "getting user reservations success",
            reservations
        })

    } catch (error) {
        console.log('Error in getting user reservations:', error)
        res.status(500).json('get user reservations error.')        
    }
}
